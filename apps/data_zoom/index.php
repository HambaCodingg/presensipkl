...
$zoom_query = mysqli_query($kon, "SELECT z.*, s.nama FROM tbl_zoom z INNER JOIN tbl_siswa s ON s.id_siswa = z.id_siswa ORDER BY z.tanggal DESC, z.waktu_awal DESC");
?>
...
<div class="row"><div class="col-md-12"><div class="panel panel-default"><div class="panel-heading"><i class="fa fa-video-camera"></i> Jadwal Zoom Internal</div><div class="panel-body">
    ...
    <?php $no = 0; while ($zoom_query && $zoom = mysqli_fetch_assoc($zoom_query)): $no++; ?>
        <tr><td><?php echo $no; ?></td><td><?php echo htmlspecialchars($zoom['nama'], ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars($zoom['judul'], ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars($zoom['tanggal'], ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars(substr($zoom['waktu_awal'], 0, 5) . ' - ' . substr($zoom['waktu_akhir'], 0, 5), ENT_QUOTES, 'UTF-8'); ?></td><td><a class="btn btn-primary btn-sm" href="apps/meeting.php?id_zoom=<?php echo (int) $zoom['id_zoom']; ?>" target="_blank"><i class="fa fa-video-camera"></i> Buka</a> <a class="btn btn-danger btn-sm" href="apps/data_zoom/hapus.php?id_zoom=<?php echo (int) $zoom['id_zoom']; ?>" onclick="return confirm('Hapus jadwal Zoom ini?')"><i class="fa fa-trash"></i></a></td></tr>
    <?php endwhile; ?>
    </tbody></table></div>
</div></div></div></div>
