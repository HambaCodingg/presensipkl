...
if (isset($_POST['simpan_zoom'])) {
    ...
    $stmt = $kon->prepare('INSERT INTO tbl_zoom (id_siswa, judul, tanggal, waktu_awal, waktu_akhir, dibuat_oleh) VALUES (?, ?, ?, ?, ?, ?)');
    $admin_id = (int) $_SESSION['id_pengguna'];
    $stmt->bind_param('issssi', $id_siswa, $judul, $tanggal, $waktu_awal, $waktu_akhir, $admin_id);
...
$siswa_query = mysqli_query($kon, 'SELECT id_siswa, nama FROM tbl_siswa ORDER BY nama ASC');
?>
<form action="apps/data_zoom/tambah.php" method="post">
    <div class="form-group"><label>Peserta Siswa</label><select name="id_siswa" class="form-control" required><?php while ($siswa = mysqli_fetch_assoc($siswa_query)): ?><option value="<?php echo (int) $siswa['id_siswa']; ?>"><?php echo htmlspecialchars($siswa['nama'], ENT_QUOTES, 'UTF-8'); ?></option><?php endwhile; ?></select></div>
